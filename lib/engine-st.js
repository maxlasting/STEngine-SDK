...
class STEngine extends Event {
  ...
  _closePeerConnection (peer) {
    const id1 = this._uid + '-' + peer
    const id2 = peer + '-' + this._uid
    if (this._connections.has(id1)) {
      const target = this._connections.get(id1)
      target.peerconnection.close()
      this._connections.delete(id1)
    }
    if (this._connections.has(id2)) {
      const target = this._connections.get(id2)
      target.peerconnection.close()
      this._connections.delete(id2)
    }
  }
}
